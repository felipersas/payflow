...
psql -v ON_ERROR_STOP=1 -U payflow -tc "SELECT 1 FROM pg_database WHERE datname = 'payflow_users'" | grep -q 1 || \
    psql -v ON_ERROR_STOP=1 -U payflow -c "CREATE DATABASE payflow_users"
echo "payflow_users: OK"
